...
function calcBatSubtotalPrice(item) {
  ...
  let subtotal = 0;
  let liftMultiplier =
    ((liftMax / liftMin - 1) / 4) * (lidarDiffFactor - 1) + 1;
  let initLiftMultiplier =
    ((initLiftMax / initLiftMin - 1) / 4) * (lidarDiffFactor - 1) + 1;
...
document
  .querySelector("#calculatePrice")
  .addEventListener("click", calcEstimatedPrice);
...
function calcEstimatedPrice() {
  ...
  document.querySelector("#estimatedPrice").innerText = estimatedPrice
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
...
function outputSubtotals() {
  ...
  for (let i = 1; i <= subtotalArray.length; ++i) {
    document.querySelector(`tbody > :nth-child(${i}) .subtotal`).innerHTML =
      subtotalArray[i - 1]
        .toFixed(2)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
document
  .querySelector("#removeLineItem")
  .addEventListener("click", removeLineItem);

function removeLineItem() {
  let rowCount = document.getElementById("list-line-item").childElementCount;
  let newRow =
    '<tr><td><input class="phase-num" type="text" value="PHASE 1"></td><td><input class="units" type="number" value=""></td><td>SEE PRICING MATRIX</td><td>$ <strong class="subtotal">0.00</strong></td></tr>';
